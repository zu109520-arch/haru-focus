...
function App() {
  ...
  const [todos, setTodos] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('haru-todos') ?? '[]');
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    document.title = isActive
      ? `(${formatTime(seconds)}) Pomodoro Timer`
      : 'Pomodoro Timer';
  }, [seconds, isActive]);
    ...
    >
      ...
      <div className="timer-card">
        ...
        <div className="controls">
          <button
            className="main-btn"
            onClick={() => setIsActive((a: boolean) => !a)}
          >
            {isActive
              ? '暫停'
              : seconds < initialSeconds && seconds > 0
                ? '繼續'
                : '開始'}
          </button>
